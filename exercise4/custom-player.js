...
function muteUnmute(){
  if(media.muted){
    ...
    //If the video is not muted, it would display the 🔉 icon.
  } else{
    media.muted = true;
    mute.setAttribute("data-icon", "🔇")
    //If video is muted, it would display the 🔇 icon.
  }
}

fullscreen.addEventListener("click", miniMax);

function miniMax(){
  //Here I define the functionality of miniMax().
  if(document.fullscreenElement == null){
    media.requestFullscreen()
    //if the video is not in fullscreen, the video will enter fullscreen mode.
  } else {
    document.exitFullscreen()
    //if the video is in fullscreen, the video will exit fullscreen mode
  }
}
//For the extra feature of the media player, I implemented a function that allows the video to enter and exit fullscreen mode.
//Since the video that I chose is of a tutorial on how to make a handheld windmill, this means that users would need to be able to look closely at what the person in the video is doing to follow the steps of the tutorial.
//without the fullscreen mode, the video was simply too small , which made it hard to see the steps of the tutorial.
//By being able to view the video in fullscreen, users would be able to look and follow the instructions clearly.